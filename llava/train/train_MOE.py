...
def find_all_linear_names(model,include_mm_projector=False):
    cls = torch.nn.Linear
    lora_module_names = set()

    #multimodal_keywords = ['vision_tower', 'vision_resampler','text_tower']
...
def smart_tokenizer_and_embedding_resize(
    ...
    """Resize tokenizer and embedding.用于智能调整tokenizer和embedding大小
    1. LLaVA需要添加特殊token（如<image>、</image>等）
    2. 原始tokenizer没有这些token
    3. embedding矩阵大小必须匹配tokenizer的词汇表大小
...
def load_model_from_previous_task(model, previous_task_model_path):
    ...
    import torch
    saved_params = torch.load(os.path.join(previous_task_model_path, 'non_lora_trainables.bin'), map_location='cpu')
    ...
    non_lora_trainables = {(k[17:] if k.startswith('base_model.') else k): v for k, v in non_lora_trainables.items()}
    if any(k.startswith('model.model.') for k in non_lora_trainables):
        non_lora_trainables = {(k[12:] if k.startswith('model.') else k): v for k, v in non_lora_trainables.items()}
    ...
    print('Loading SAME...')
    same_state_path = os.path.join(previous_task_model_path, 'same_state.bin')
    if os.path.exists(same_state_path):
        same_state_dict = torch.load(same_state_path, map_location='cpu')
        valid_raw = 0
        for name, param in same_state_dict.items():
            if 'cov_prev_valid' in name:
                if hasattr(param, 'item') and param.item():
                    valid_raw += 1
        model_valid_before = 0
        for name, param in model.named_buffers():
            if 'cov_prev_valid' in name:
                if param.item():
                    model_valid_before += 1
        try:
            model.load_state_dict(same_state_dict, strict=False)
            print("Direct loading successful")
        except Exception as e:
            print(f"Direct loading failed: {e}")
            loaded_manual = 0
            for name, param in same_state_dict.items():
                if 'cov_prev_valid' in name or 'cov_U_prev' in name or 'cov_S_prev' in name or 'importance' in name:
                    try:
                        clean_name = name
                        ...
                        for module_name, module in model.named_modules():
                            if isinstance(module, SAMELinear):
                                expected_names = [
                                    f"{module_name}.cov_U_prev_{module.active_adapter}",
                                    f"{module_name}.cov_S_prev_{module.active_adapter}", 
                                    f"{module_name}.cov_prev_valid_{module.active_adapter}",
                                    f"{module_name}.importance_{module.active_adapter}"
                                ]
                                if clean_name in expected_names:
                                    buffer_name = clean_name.split('.')[-1]
                                    setattr(module, buffer_name, param)
                                    loaded_manual += 1
                                    found = True
                                    break
                        if not found:
                            print(f"Could not find module for: {clean_name}")
                    except Exception as e2:
                        print(f"Failed to set {name}: {e2}")
    else:
        print("⚠️ SAME state not found")
...
def train():
    ...
    parser = transformers.HfArgumentParser((m_ModelArguments, DataArguments, TrainingArguments))
    ...
    if training_args.lora_enable:
        ...
        lora_config = SAMEConfig(
            r=training_args.lora_r,
            lora_alpha=training_args.lora_alpha,
            target_modules=find_all_linear_names(model),
            lora_dropout=training_args.lora_dropout,
            bias=training_args.lora_bias,
            task_type=TaskType.CAUSAL_LM_SAME,
            **kwargs
        )
    ...
    print("=== Calling save_task_covariance_snapshot ===")
    for module in model.modules():
        if isinstance(module, SAMELinear):
            module.save_task_covariance_snapshot(module.active_adapter)
    print("=== Finished saving covariance snapshots ===")
    ...
    for name, param in model.named_parameters():
        ...
        if is_target or param.requires_grad:
            saved_param = param.data.clone()
            saved_param.requires_grad = param.requires_grad
            extra_save[name] = saved_param


    same_state_dict = {}
    for name, param in model.named_buffers():
        if any(kw in name for kw in ['cov_U_prev', 'cov_S_prev', 'importance','cov_prev_valid']):
            same_state_dict[name] = param

    torch.save(same_state_dict, os.path.join(training_args.output_dir, 'same_state.bin'))


    if training_args.lora_enable:
        model.set_prototype_params_for_save()
        ...
        if training_args.local_rank == 0 or training_args.local_rank == -1:
            ...
            torch.save(extra_save, os.path.join(training_args.output_dir, 'non_lora_trainables.bin'))
    else:
