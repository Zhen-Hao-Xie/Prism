...
2. **框架内建（推荐复现作者）**：未设 ``ins_emb_path`` 且 ``smolora_builtin_sentence_ins_emb`` 为 True 时，
   ...
   ``method.smolora.builtin_ins_emb.INS_EMB_SINGLE_INSTRUCTIONS`` 做 mean pooling，等价于单独运行
...
def ensure_peft_extension_registered() -> None:
    ...
    if _PEFT_EXT_REGISTERED:
        return
    from PEFT.peft.tuners.smolora import SMoLoraConfig, SMoLoraModel
...
class SmoloraIntegration(CLIntegration):
    def __init__(self, config: Any):
        ...
        if self._ins_emb_path:
            ...
            if self._use_builtin_sentence_ins_emb:
                ...
                self.ins_emb_dim: int = int(getattr(config, "clip_feature_dim", getattr(config, "ins_emb_dim", 768)))
    ...
    def initialize_model(self, model) -> None:
        ...
        from PEFT.peft.tuners.smolora import SMoLoraConfig
        from PEFT.peft.utils import TaskType
        ...
        if self._use_builtin_sentence_ins_emb and self._author_ins_emb_list is None:
            from method.smolora.builtin_ins_emb import compute_default_ins_emb_matrix
    ...
    def _find_target_modules(self, model) -> List[str]:
        """
        只收集语言模型里的 attention Linear；排除 CLIP ``text_tower`` / ``vision_tower`` 等，
        否则 ``q_proj`` 会与 CLIP 内层同名一起被 PEFT 替换，导致在 ``on_input_prep`` 里跑 CLIP 时
        先进入尚未写入 ``_runtime_instruction_feat`` 的 SMoLoraLinear 而报错。
        """
    ...
    def _propagate_ins_type(self, model: Any, ins_type: int) -> None:
        from PEFT.peft.tuners.smolora import SMoLoraLinear
